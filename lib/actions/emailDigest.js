...
// User must be imported here so Mongoose registers its schema before
// Halaqah.populate("members.userId") runs. In Vercel serverless each
// invocation is isolated, so without this import populate() throws
...
export async function getAllHalaqahDigestData() {
  ...
  for (const halaqah of halaqahs) {
    ...
    for (const member of halaqah.members) {
      ...
      const unsubToken = await generateUnsubscribeToken(userId, halaqahId);
      ...
      emails.push({
        ...
        links: {
          circle: `${appUrl}/halaqah/${halaqahId}`,
