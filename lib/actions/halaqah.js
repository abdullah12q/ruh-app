...
export async function getUserHalaqahs() {
  try {
    ...
    const serialised = halaqahs.map((h) => ({
      ...
      emailOptOut:
        h.members.find((m) => m.userId.toString() === userId)?.emailOptOut ??
        false,
    }));
...
export async function toggleEmailSubscription(halaqahId, optOut) {
  try {
    const userId = await getAuthenticatedUserId();

    await connectDB();

    const updated = await Halaqah.findOneAndUpdate(
      { _id: halaqahId, "members.userId": userId },
      { $set: { "members.$.emailOptOut": optOut } },
      { returnDocument: "after", runValidators: true },
    );
    ...
    if (err?.success === false) return err;
    console.error("[toggleEmailSubscription]", err);
    return { success: false, error: "Something went wrong. Please try again." };
  }
}
