...
export function useEnglishTranslation(surahId, ayahNum) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok)
        throw new Error(`quranenc EN error: ${res.status}`);
      const data = await res.json();
      // API returns { result: { translation, footnotes, ... } }
      return data?.result?.translation ?? null;
    },
...
/**
 * useArabicTranslation — Fetches Al-Muyassar Arabic tafsir
...
export function useArabicTranslation(surahId, ayahNum) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://cdn.jsdelivr.net/gh/spa5k/tafsir_api@main/tafsir/al-muyassar-fi-al-gharib/${surahId}/${ayahNum}.json`,
      );
      if (!res.ok)
        throw new Error(`tafsir AR error: ${res.status}`);
      const data = await res.json();
      // API returns { text, ... }
...
export function useAyahTranslation(surahId, ayahNum) {
  ...
  if (lang === "ar") {
    return { text: ar.data, isLoading: ar.isLoading, isError: ar.isError, lang };
  }
  return { text: en.data, isLoading: en.isLoading, isError: en.isError, lang };
}
