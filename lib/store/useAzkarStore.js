...
const useAzkarStore = create(
  persist(
    (set, get) => ({
      ...
      hydrateFromServer: (serverProgress) =>
        set((state) => {
          ...
          const mergedProgress = { ...freshProgress };

          Object.keys(freshProgress).forEach((key) => {
            const serverItem = serverProgress[key];
            const freshItem = freshProgress[key];

            if (serverItem && serverItem.total === freshItem.total) {
              mergedProgress[key] = serverItem;
            }
          });
    ...
    }),
    {
      ...
      // Merge fresh defaults with persisted progress to handle new/removed entries
      merge: (persistedState, currentState) => {
        const freshProgress = buildInitialProgress();
        const mergedProgress = { ...freshProgress };

        if (persistedState?.progress) {
          // Only iterate over the fresh keys to avoid keeping deleted azkar
          Object.keys(freshProgress).forEach((key) => {
            const persistedItem = persistedState.progress[key];
            const freshItem = freshProgress[key];

            if (persistedItem) {
              // If the total hasn't changed, restore the user's progress
              if (persistedItem.total === freshItem.total) {
                mergedProgress[key] = persistedItem;
              }
              // If the total has changed, let the freshItem overwrite it
              // to prevent progress bugs (e.g., remaining > total).
            }
          });
        }
