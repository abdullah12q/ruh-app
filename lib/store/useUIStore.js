 ...
 * - isExpandedReadingControls (persisted)
 */
    ...
    (set) => ({
      ...
      hydrateFromServer: (serverPrefs) =>
        set((state) => {
          ...
          // UI State
          if (serverPrefs.uiState) {
            if (serverPrefs.uiState.fontSize)
              updates.fontSize = serverPrefs.uiState.fontSize;
            if (serverPrefs.uiState.translationLanguage)
              updates.translationLang = serverPrefs.uiState.translationLanguage;
            if (serverPrefs.uiState.mushafMode !== undefined)
              updates.mushafMode = serverPrefs.uiState.mushafMode;
            if (serverPrefs.uiState.autoScrollToNextAyah !== undefined)
              updates.autoScrollToNextAyah =
                serverPrefs.uiState.autoScrollToNextAyah;
          }

          // Audio State
          if (serverPrefs.audioState) {
            if (serverPrefs.audioState.favoriteReciters)
              updates.favoriteReciters =
                serverPrefs.audioState.favoriteReciters;
            if (serverPrefs.audioState.selectedReciter) {
              const reciter = imamList.find(
                (r) => r.id === serverPrefs.audioState.selectedReciter,
              );
              if (reciter) updates.selectedReciter = reciter;
            }
          }

          // Prayer Config
          if (serverPrefs.prayerConfig) {
            if (serverPrefs.prayerConfig.prayerMethod !== undefined)
              updates.prayerCalcMethod =
                serverPrefs.prayerConfig.prayerCalcMethod;
            if (serverPrefs.prayerConfig.dstAdjustment !== undefined)
              updates.dstAdjustment = serverPrefs.prayerConfig.dstAdjustment;
            if (serverPrefs.prayerConfig.prayerNotificationMode)
              updates.prayerNotificationMode =
                serverPrefs.prayerConfig.prayerNotificationMode;
            if (serverPrefs.prayerConfig.prayerNotificationOffsets)
              updates.prayerNotificationOffsets =
                serverPrefs.prayerConfig.prayerNotificationOffsets;
          }

          // Bookmarks
          if (serverPrefs.bookmarkedAyahs?.length > 0) {
            updates.bookmarkedAyahs = serverPrefs.bookmarkedAyahs
              .map((entry) => {
                if (typeof entry === "string") {
                  const [surahNum, ayahNum] = entry.split(":").map(Number);
                  return { surahNum, ayahNum };
                }
                return entry; // already an object (e.g. local state)
              })
              .filter((b) => !isNaN(b.surahNum) && !isNaN(b.ayahNum));
          }

          // Reading Progress
          if (
            serverPrefs.readingProgress &&
            serverPrefs.readingProgress.length > 0
          ) {
            const serverReads = serverPrefs.readingProgress.map((r) => ({
              surahId: r.surahId,
              ayahNumber: r.ayahNumber,
              reciter: r.reciter ?? null,
              mushafMode: r.mushafMode ?? false,
              currentPage: r.currentPage ?? 1,
            }));

            updates.recentReads = serverReads;

            const latest = serverReads[0];
            if (
              !state.lastRead ||
              state.lastRead.surahId !== latest.surahId ||
              state.lastRead.ayahNumber !== latest.ayahNumber
            ) {
              updates.lastRead = {
              ...
              updates.activeAyah = {
                ...(state.activeAyah || {}),
                [latest.surahId]: latest.ayahNumber,
              };
            }
          }
      ...
      // --- Reading Controls Expanded State ---
      isExpandedReadingControls: true,
      toggleIsExpandedReadingControls: () =>
        set((s) => ({
          isExpandedReadingControls: !s.isExpandedReadingControls,
        })),
    }),
    {
      ...
      partialize: (state) => ({
        ...
        isExpandedReadingControls: state.isExpandedReadingControls,
      }),
    },
  ),
);
