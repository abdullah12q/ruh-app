...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  providers: [
    ...
    Google({
      ...
      profile(profile) {
        return {
          id: profile.sub, // 'sub' is Google's unique ID for the user
          name: profile.name,
          studyCircleName: profile.name,
          email: profile.email,
          // Google sometimes puts the image in 'picture', sometimes in 'image'
    ...
    Credentials({
      ...
      async authorize(credentials) {
        ...
        if (!user || !user.password) {
          throw new CustomAuthError("Incorrect email or password");
        }
        ...
        return {
          ...
          email: user.email,
          name: user.name,
          studyCircleName: user.studyCircleName || user.name,
          image: user.image,
        };
      },
    }),
      ...
      if (account?.provider === "google" && account.access_token) {
        try {
          ...
          if (googleProfile.picture) {
            ...
            // Update the user in the database so the image is available
            if (token.userId) {
              await connectDB();
