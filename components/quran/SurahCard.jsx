...
export default function SurahCard({ surah, index }) {
  ...
  return (
    <Link
      href={`/quran/${surah.id}`}
      className="group relative overflow-hidden glass rounded-2xl p-5 flex items-center gap-4 hover:border-(--accent)/30! transition-all duration-900 hover:shadow-[0_0_20px_rgba(20,184,166,0.1)]!"
    >
      {/* Surah Number Badge */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          ...
          <span
            aria-hidden
            className="pointer-events-none select-none absolute right-0.5 -top-4 font-quran text-[2.5rem] sm:text-[3rem] leading-none text-accent opacity-50 group-hover:opacity-100 group-hover:-translate-x-1 transition-all duration-500 whitespace-nowrap"
          >
            {surah.name_arabic}
          </span>
        </div>
